...
describe('findVanity — multiple successive hits', () => {
    it('three successive .next() calls each return a valid, noble-confirmed, distinct hit', async () => {
        ...
        const seedKeys = hits.map(h => Array.from(h.seed).join(','));
        expect(new Set(seedKeys).size).toBe(3);
    }, 15_000);
});
